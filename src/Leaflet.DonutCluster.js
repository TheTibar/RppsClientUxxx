...
(function (factory, window) {
    ...
    function donut(options) {
        var div = doc.createElement('div'),
            ...
            size = options.size, //taille du donut, ne sert que dans ces variables, est remplacé par size0 pour le svg ou r pour le donut
            size0 = size + 10, //permet de "grossir" sur l'arc de 5 pixels
            data = options.data || [{
                value: 1
            }],
            innerCircleRadius = options.innerCircleRadius,
            //weight = options.weight || 20,
            //weight = options.weight, //épaisseur de la bordure du donut
            weight = 8, //épaisseur de la bordure du donut
            colors = options.colors || ['#555'],
            fillColor = options.fillColor || '#f1d357',
            el = options.el,
            r = size / 2, //rayon exterieur du donut
            //r = size,
            PI = M.PI,
            sin = M.sin,
            cos = M.cos,
        ...
        */
        ...
        circle.setAttribute('r', innerCircleRadius);
        ...
        svg.innerHTML = '<circle cx="' + size0 / 2.0 + '" cy="' + size0 / 2.0 + '" r="' + innerCircleRadius + '" fill="' + fillColor + '" fill-opacity="0.6"></circle>'
        //console.log(circle);
        
        div.appendChild(svg);

        for (i = 0; i < data.length; i++) {
            //value = data[i].value / sum; //orig
            value = sum > 0 ? data[i].value / sum : 0; //new
            value = value === 1 ? .99999 : value;
            arc = doc.createElementNS(NS, 'path');
            var r1 = r + 5; //permet de "zoomer" sur l'arc défini
            ...
            (function (d, c, perc) {
                ...
                arc.addEventListener('mouseenter', function (e) {
                    ...
                    t.saved = {
                        ...
                        //legend: '<span style="border: 1px solid ' + c + '; background-color:rgba(255, 255, 255, 0.7) ;border-left-width:15px; padding:1px; white-space: nowrap;">' + (d.title || d.name) + ':' + perc + '%</span>'
                        legend: '<span style="border: 1px solid ' + c + '; background-color:rgba(255, 255, 255, 0.7) ;border-left-width:15px; padding:1px; white-space: nowrap;">' + d.title + ' : ' + perc + '%</span>'
                    }
    ...
    function createDonut(points, opt, cfgFn) {
        ...
        var innerCircleRadius = 15, //ajout XB
        	weight = Math.sqrt(sum(blocks)), //modif XB, correspond à l'épaisseur du donut
        	//weight = cfg.weight || 10,
        	size = 2 * innerCircleRadius + 2 * weight, //modif XB, le svg du donut a pour taille 2 * le rayon du cercle intérieur + 2 * l'épaisseur du donut 
        	//size = cfg.size || 50,
            colors = cfg.colors;
        var myDonut = donut({
            size: size,
    ...
    function defaultStyle(points) {
    	//sert pour le total au centre du donut
